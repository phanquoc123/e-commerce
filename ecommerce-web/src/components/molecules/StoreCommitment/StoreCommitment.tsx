...
export default function StoreCommitment({ 
  ...
  return (
    <div className="space-y-2 rounded bg-white px-3 py-6 border-b border-border-primary">
      <p className="text-theme-text text-heading-md">{title}</p>
      
      {commitments.map((commitment, index) => (
        <div key={index} className="flex items-center gap-2.5">
          <div className="rounded-sm bg-theme-surface p-2">
            <svg 
      ...
      <div className="h-px w-full border-b border-dashed border-border-primary"></div>
    </div>
  );
}
